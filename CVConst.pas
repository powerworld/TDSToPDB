...
const
  CV_CALL_NEAR_C      = $00;  // near right to left push, caller pops stack
  CV_CALL_FAR_C       = $01;  // far right to left push, caller pops stack
  CV_CALL_NEAR_PASCAL = $02;  // near left to right push, callee pops stack
  CV_CALL_FAR_PASCAL  = $03;  // far left to right push, callee pops stack
  CV_CALL_NEAR_FAST   = $04;  // near left to right push with regs, callee pops stack
  CV_CALL_FAR_FAST    = $05;  // far left to right push with regs, callee pops stack
  CV_CALL_SKIPPED     = $06;  // skipped (unused) call index
  CV_CALL_NEAR_STD    = $07;  // near standard call
  CV_CALL_FAR_STD     = $08;  // far standard call
  CV_CALL_NEAR_SYS    = $09;  // near sys call
  CV_CALL_FAR_SYS     = $0a;  // far sys call
  CV_CALL_THISCALL    = $0b;  // this call (this passed in register)
  CV_CALL_MIPSCALL    = $0c;  // Mips call
  CV_CALL_GENERIC     = $0d;  // Generic call sequence
  CV_CALL_ALPHACALL   = $0e;  // Alpha call
  CV_CALL_PPCCALL     = $0f;  // PPC call
  CV_CALL_SHCALL      = $10;  // Hitachi SuperH call
  CV_CALL_ARMCALL     = $11;  // ARM call
  CV_CALL_AM33CALL    = $12;  // AM33 call
  CV_CALL_TRICALL     = $13;  // TriCore Call
  CV_CALL_SH5CALL     = $14;  // Hitachi SuperH-5 call
  CV_CALL_M32RCALL    = $15;  // M32R Call
  CV_CALL_CLRCALL     = $16;  // clr call
  CV_CALL_INLINE      = $17;  // Marker for routines always inlined and thus lacking a convention
  CV_CALL_NEAR_VECTOR = $18;  // near left to right push with regs, callee pops stack
  CV_CALL_RESERVED    = $19;  // first unused call enumeration
...
//      Values for the access protection of class attributes
  ...
  THUNK_ORDINAL_LOAD               = 4; // thunk which loads the address to jump to
                                        //  via unknown means...
...
//      enumeration for type modifier values
  ...
  CV_MOD_CONST                        = $0001;
  CV_MOD_VOLATILE                     = $0002;
  CV_MOD_UNALIGNED                    = $0003;

  // 0x0200 - 0x03ff - HLSL modifiers.

  CV_MOD_HLSL_UNIFORM                 = $0200;
  CV_MOD_HLSL_LINE                    = $0201;
  CV_MOD_HLSL_TRIANGLE                = $0202;
  CV_MOD_HLSL_LINEADJ                 = $0203;
  CV_MOD_HLSL_TRIANGLEADJ             = $0204;
  CV_MOD_HLSL_LINEAR                  = $0205;
  CV_MOD_HLSL_CENTROID                = $0206;
  CV_MOD_HLSL_CONSTINTERP             = $0207;
  CV_MOD_HLSL_NOPERSPECTIVE           = $0208;
  CV_MOD_HLSL_SAMPLE                  = $0209;
  CV_MOD_HLSL_CENTER                  = $020a;
  CV_MOD_HLSL_SNORM                   = $020b;
  CV_MOD_HLSL_UNORM                   = $020c;
  CV_MOD_HLSL_PRECISE                 = $020d;
  CV_MOD_HLSL_UAV_GLOBALLY_COHERENT   = $020e;
...
//      built-in type kinds
  ...
  CV_BI_HLSL_INTERFACE_POINTER        = $0200;
  CV_BI_HLSL_TEXTURE1D                = $0201;
  CV_BI_HLSL_TEXTURE1D_ARRAY          = $0202;
  CV_BI_HLSL_TEXTURE2D                = $0203;
  CV_BI_HLSL_TEXTURE2D_ARRAY          = $0204;
  CV_BI_HLSL_TEXTURE3D                = $0205;
  CV_BI_HLSL_TEXTURECUBE              = $0206;
  CV_BI_HLSL_TEXTURECUBE_ARRAY        = $0207;
  CV_BI_HLSL_TEXTURE2DMS              = $0208;
  CV_BI_HLSL_TEXTURE2DMS_ARRAY        = $0209;
  CV_BI_HLSL_SAMPLER                  = $020a;
  CV_BI_HLSL_SAMPLERCOMPARISON        = $020b;
  CV_BI_HLSL_BUFFER                   = $020c;
  CV_BI_HLSL_POINTSTREAM              = $020d;
  CV_BI_HLSL_LINESTREAM               = $020e;
  CV_BI_HLSL_TRIANGLESTREAM           = $020f;
  CV_BI_HLSL_INPUTPATCH               = $0210;
  CV_BI_HLSL_OUTPUTPATCH              = $0211;
  CV_BI_HLSL_RWTEXTURE1D              = $0212;
  CV_BI_HLSL_RWTEXTURE1D_ARRAY        = $0213;
  CV_BI_HLSL_RWTEXTURE2D              = $0214;
  CV_BI_HLSL_RWTEXTURE2D_ARRAY        = $0215;
  CV_BI_HLSL_RWTEXTURE3D              = $0216;
  CV_BI_HLSL_RWBUFFER                 = $0217;
  CV_BI_HLSL_BYTEADDRESS_BUFFER       = $0218;
  CV_BI_HLSL_RWBYTEADDRESS_BUFFER     = $0219;
  CV_BI_HLSL_STRUCTURED_BUFFER        = $021a;
  CV_BI_HLSL_RWSTRUCTURED_BUFFER      = $021b;
  CV_BI_HLSL_APPEND_STRUCTURED_BUFFER = $021c;
  CV_BI_HLSL_CONSUME_STRUCTURED_BUFFER= $021d;
  CV_BI_HLSL_MIN8FLOAT                = $021e;
  CV_BI_HLSL_MIN10FLOAT               = $021f;
  CV_BI_HLSL_MIN16FLOAT               = $0220;
  CV_BI_HLSL_MIN12INT                 = $0221;
  CV_BI_HLSL_MIN16INT                 = $0222;
  CV_BI_HLSL_MIN16UINT                = $0223;
...
//  enum describing the compile flag source language
  ...
  CV_CFL_CVTPGD   = $09;
  CV_CFL_CSHARP   = $0A;  // C#
  CV_CFL_VB       = $0B;  // Visual Basic
  CV_CFL_ILASM    = $0C;  // IL (as in CLR) ASM
  CV_CFL_JAVA     = $0D;
  CV_CFL_JSCRIPT  = $0E;
  CV_CFL_MSIL     = $0F;  // Unknown MSIL (LTCG of .NETMODULE)
  CV_CFL_HLSL     = $10;   // High Level Shader Language
...
//  enum describing target processor
  ...
  *)
  ...
  FrameTypeFPO = 0;               // Frame pointer omitted, FPO info available
  FrameTypeTrap = 1;              // Kernel Trap frame
  FrameTypeTSS = 2;               // Kernel Trap frame
  FrameTypeStandard = 3;          // Standard EBP stackframe
  FrameTypeFrameData = 4;         // Frame pointer omitted, FrameData info available

  FrameTypeUnknown = -1;          // Frame which does not have any debug info

  MemTypeCode = 0;                // Read only code memory
  MemTypeData = 1;                // Read only data/stack memory
  MemTypeStack = 2;               // Read only stack memory
  MemTypeCodeOnHeap = 3;          // Read only memory for code generated on heap by runtime

  MemTypeAny = -1;

  // HLSL specific memory spaces

  CV_HLSL_MEMSPACE_DATA         = $00;
  CV_HLSL_MEMSPACE_SAMPLER      = $01;
  CV_HLSL_MEMSPACE_RESOURCE     = $02;
  CV_HLSL_MEMSPACE_RWRESOURCE   = $03;

  CV_HLSL_MEMSPACE_MAX          = $0F;
